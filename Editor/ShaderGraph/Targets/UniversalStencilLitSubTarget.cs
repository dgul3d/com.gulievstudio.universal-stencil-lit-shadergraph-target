...
{
    sealed class UniversalStencilLitSubTarget : UniversalSubTarget, ILegacyTarget
    {
        static readonly GUID kSourceCodeGuid = new GUID("d6c78107b64145745805d963de80cc29"); // UniversalStencilLitSubTarget.cs
        ...
        public UniversalStencilLitSubTarget()
        {
            displayName = "Stencil Lit";
        }
        ...
        public override void Setup(ref TargetSetupContext context)
        {
            ...
            {
                var gui = typeof(ShaderGraphStencilLitGUI);
#if HAS_VFX_GRAPH
                if (TargetsVFX())
                    gui = typeof(VFXShaderGraphStencilLitGUI);
#endif
                context.AddCustomEditorForRenderPipeline(gui.FullName, universalRPType);
            }

            // Process SubShaders
            context.AddSubShader(PostProcessSubShader(SubShaders.LitSubShader(target, workflowMode, target.renderType, target.renderQueue, target.disableBatching, complexLit, blendModePreserveSpecular, enableStencil)));
        }
        ...
        public override void ProcessPreviewMaterial(Material material)
        {
            ...
            ShaderGraphStencilLitGUI.UpdateMaterial(material, MaterialUpdateType.CreatedNewMaterial);
        }
        ...
        {
            public static SubShaderDescriptor LitSubShader(UniversalTarget target, WorkflowMode workflowMode, string renderType, string renderQueue, string disableBatchingTag, bool complexLit, bool blendModePreserveSpecular, bool enableStencil)
            {
                ...
                if (complexLit)
                    result.passes.Add(LitPasses.ForwardOnly(target, workflowMode, complexLit, blendModePreserveSpecular, enableStencil, CoreBlockMasks.Vertex, LitBlockMasks.FragmentComplexLit, CorePragmas.Forward, LitKeywords.Forward));
                else
                    result.passes.Add(LitPasses.Forward(target, workflowMode, blendModePreserveSpecular, enableStencil, CorePragmas.Forward, LitKeywords.Forward));

                // ForwardOnly ComplexLit fills GBuffer too for potential custom usage of the GBuffer.
                result.passes.Add(LitPasses.GBuffer(target, workflowMode, blendModePreserveSpecular, enableStencil));
                ...
                if (complexLit)
                    result.passes.Add(PassVariant(LitPasses.DepthNormalOnly(target, enableStencil), CorePragmas.Instanced));
                else
                    result.passes.Add(PassVariant(LitPasses.DepthNormal(target, enableStencil), CorePragmas.Instanced));

                result.passes.Add(PassVariant(LitPasses.Meta(target, enableStencil), CorePragmas.Default));
                ...
                result.passes.Add(PassVariant(LitPasses._2D(target, enableStencil), CorePragmas.Default));
        ...
        static class LitPasses
        {
            ...
            public static PassDescriptor GBuffer(UniversalTarget target, WorkflowMode workflowMode, bool blendModePreserveSpecular, bool enableStencil)
            {
                var result = new PassDescriptor
                {
                    // Definition
            ...
            public static PassDescriptor Meta(UniversalTarget target, bool enableStencil)
            {
                var result = new PassDescriptor()
                {
                    // Definition
            ...
            public static PassDescriptor _2D(UniversalTarget target, bool enableStencil)
            {
                var result = new PassDescriptor()
                {
                    // Definition
            ...
            public static PassDescriptor DepthNormal(UniversalTarget target, bool enableStencil)
            {
                var result = new PassDescriptor()
                {
                    // Definition
